...
public class SlotRepository(
    ...
    IOptions<SlotServiceOptions> slotServiceOptions,
    ...
    : ISlotRepository
{
    ...
    public async Task<BusySlotsSchedule> GetBusySlotsScheduleAsync(DateTimeOffset date)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            logger.LogError("Request Get Busy Slots failed with status code {StatusCode}. {ExceptionMessage}",
                response.StatusCode, exceptionMessage);
            throw new HttpRequestException(
                $"Request failed with status code {response.StatusCode}. {exceptionMessage}");
        }
    ...
    public async Task<HttpStatusCode> BookSlotAsync(AvailableSlot availableSlot)
    {
        ...
        request.Content =
            new StringContent(JsonConvert.SerializeObject(availableSlot), Encoding.UTF8, "application/json");
        AddAuthorizationHeader(httpClient);
        ...
        {
            var exceptionMessage = await response.Content.ReadAsStringAsync();
            logger.LogError("Request Book Available Slots failed with status code {StatusCode}. {ExceptionMessage}",
                response.StatusCode, exceptionMessage);
            throw new HttpRequestException(
                $"Request failed with status code {response.StatusCode}. {exceptionMessage}");
        }
